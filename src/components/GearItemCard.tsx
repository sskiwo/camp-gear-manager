      ...
      {isEditing ? (
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5">
            ...
            <input type="number" value={editWeight} onChange={(e) => setEditWeight(e.target.value)} className="px-2 py-1 border border-zinc-700 rounded bg-[#18181B] text-white text-xs" placeholder="重量(g)" />
          ...
          <div className="flex items-center justify-between pt-1">
            ...
            <div className="flex gap-2">
              <button onClick={handleSave} className="bg-[#FF5500] text-white px-3 py-1 rounded text-[11px] font-bold hover:bg-[#E04B00]">保存</button>
              <button onClick={() => setIsEditing(false)} className="bg-zinc-700 text-zinc-300 px-3 py-1 rounded text-[11px] font-bold">キャンセル</button>
            </div>
          </div>
        </div>
      ) : (
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5">
          ...
          <div className="flex items-center justify-between sm:justify-end gap-2 text-[11px] pl-8 sm:pl-0">
            ...
            <button onClick={() => setIsEditing(true)} className="text-zinc-400 hover:text-white" title="編集">✏️</button>
